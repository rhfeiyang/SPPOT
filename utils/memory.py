...
class LogitsMemory(torch.nn.Module):
    ...
    def __init__(self, dim=10, size=5120):
        ...
        self.register_buffer('data_idx', torch.LongTensor(self.size),persistent=False)
        print('Bank shape: ({},{})'.format(self.size, dim))
    @torch.no_grad()
    def forward(self, input_logits=None, update=True, enqueue=True, data_idxs=None):
        if update:
            if not enqueue:
                self.index-=input_logits.size(0)
            idx = self.update_memory(input_logits,data_idxs)
        else:
            idx = self.index
        return self.memory, self.data_idx ,idx

    @torch.no_grad()
    def update_memory(self, input, data_idxs):
        # update memory
        ...
        self.memory.index_copy_(0, out_ids, input)
        self.data_idx.index_copy_(0, out_ids, data_idxs.cuda(non_blocking=True))
    ...
    def init(self, data_loader, model, head=0):
        ...
        for i, batch in enumerate(data_loader):
            if "image" in batch:
                images = batch['image'][0].cuda(non_blocking=True) if isinstance(batch['image'], list) else batch['image'].cuda(non_blocking=True)
            elif "anchor" in batch:
                images = batch['anchor'][0].cuda(non_blocking=True) if isinstance(batch['anchor'], list) else batch['anchor'].cuda(non_blocking=True)
            else:
                raise ValueError("image or anchor not in batch")
            logits = model(images)[head]
            idx = batch['meta']['index']
            self.update_memory(logits,idx)
            if i == iter_num-1:
                break
        self.is_inited = True
        print('Memory bank inited')
        return self

class FeatureMemory(LogitsMemory):
    def __init__(self, dim=768, size=5120):
        super(FeatureMemory, self).__init__(dim=dim, size=size)

    @torch.no_grad()
    def init(self, data_loader, model):
        assert not self.is_inited
        assert self.size<=len(data_loader.dataset)
        # init logits bank, by randomly self.size logits
        model.eval()
        ...
        for i, batch in enumerate(data_loader):
            if "image" in batch:
                ...
                raise ValueError("image or anchor not in batch")
            feat = model(images, forward_pass = "backbone")
            idx = batch['meta']['index']
            self.update_memory(feat, idx)
            if i == iter_num-1:
                break
        self.is_inited = True
        print('feature bank inited')
        return self