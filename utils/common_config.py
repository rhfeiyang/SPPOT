...
def get_train_dataloader(p, dataset):
    determine = "seed" in p and p["seed"] is not None
    g=None
    if determine:
        g=torch.Generator()
        g.manual_seed(p["seed"])
        p.trainloader_generator=g
    return torch.utils.data.DataLoader(dataset, num_workers=p['num_workers'], 
            batch_size=p['batch_size'], pin_memory=False, collate_fn=collate_custom,
            drop_last=True, shuffle=True,
            worker_init_fn=seed_worker if determine else None,
            generator=g)


def get_val_dataloader(p, dataset):
    return torch.utils.data.DataLoader(dataset, num_workers=p.num_workers,
            batch_size=p.eval_batch_size, pin_memory=False, collate_fn=collate_custom,
...
def get_train_transformations(p):
    if p['backbone'] == 'dino_vitb16':
        ...
        from data.wrapper import Two_view_transform
        ...
        return Two_view_transform(transform)
    else:
