...
def get_predictions(p, dataloader, model):
    ...
    for batch in dataloader:
        ...
        output = res['output']
        features.append(res['features'].detach().cpu())
    ...
    out = [{'predictions': pred_, 'probabilities': prob_, "logits": logits[i]} for i,(pred_, prob_) in enumerate(zip(predictions, probs))]

    out.append({'targets': targets, 'features': torch.cat(features, dim=0)})
    return out
...
def hungarian_evaluate(subhead_index, all_predictions, num_classes=None, class_names=None,
    ...
    targets = all_predictions[-1]['targets'].cuda()
    ...
    return {'mACC': round(balance_acc*100,2),'F1': round(f1_score*100,2),'ARI': round(ari*100,2), 'NMI': round(nmi*100,2),
            'Precision': round(precision*100,2), 'class_acc': class_acc,"ACC_head_medium_tail": acc_analyze, 'class_f1': class_f1 ,
            'class_precision': class_precision, 'hungarian_match': match if len(match) <= 10 else "omit"}
...
def get_feature(p, dataloader, model, save_dir=None):
    save_path = os.path.join(save_dir, f"init_feature_result_{p['train_db_name']}_{p['num_classes'][0]}_{p['imbalance_ratio']}.pth")
